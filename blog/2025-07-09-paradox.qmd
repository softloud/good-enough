...
Consulting and choosing the correct statistical approach for diverse problems is unquestionably a challenging conceptual task. 
...
> The dirty secret a the heart of data science that leadership are struggling to grasp is this: getting table $X$ is never easy, it's always hard. 

Instead, we have a set of tables, let's call them $[Z]$ wherein each table may arrive not on in a different shape but even in different formats. 

And the transformation pipeline to shape $[Z]$ to $X$ represents the vast proportion of development work required. This process can be conceptualised as one enormous directed acyclic graph. 
...
Worse still, the entire team need to understand the $X$ desired, and that $X$ is subject to change as decisions are made about the desired $Y$ during development. Increasingly, it's rare for the people getting the $X$ and computing the $Y$ to be the same people, confusions abound. 

---

Without tests and validation, we find that a butterfly of shit data can flap its wings and cause a tornado of broken analyses, trauma, and workplace toxicity. 
